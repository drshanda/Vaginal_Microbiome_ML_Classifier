...
Data was filtered so that an feature/OTU must appear in ≥5 samples and they must have ≥10 total reads across dataset.
```{r}
min_prevalence <- 5 
min_count <- 10    
...
PERMANOVA using CLR-transformed Aitchison distances demonstrated highly significant differences in microbiome composition between clinical groups. Microbial community structure differed significantly between at least two clinical groups (p = 0.001). ~13.5% of variation in global microbiome composition is explained by clinical classification (R² = 0.1348).      
...
```
A BETADISPER test was performed to ensure that PERMANOVA differences reflect true centroid separation and are not artifacts of differing dispersion. There is significant difference in dispersion between groups (p = 0.003). BV samples exhibited substantially greater dispersion compared with healthy or VVC samples, consistent with the known ecological instability of BV-associated microbiota.
...
A Kruskal-Wallis test was performed to identify statistically significant differences in the relative abundance of the top 5 biomarkers in the dataset: CLR_1, CLR_14, CLR_17, CLR_3, and CLR_43. Across the top 5 biomarkers, there is a statistical significant result (p < 2.2e-16) indicating at least one group has a different microbial abundance compared to the others. Post-Hoc tests will be conducted next to determine which groups differ across the selected taxa. 
...
kw_results <- df_full %>%
    ...
    group_by(Feature) %>%
    kruskal.test(Value ~ Status_Code)

print(kw_results)
```

 
### 3.2 Post-Hoc Dunn's Test
...
### 3.3 Visualization of Biomarker Distributions
...
### 3.4 Pairwise Mann-Whitney U (Wilcoxon Rank-Sum) Tests (FDR Corrected)
...
* **F = 6.0969, p = 0.003** for multivariate dispersion
